                    ...
                    <div id="blogPostsContainer">
                        @foreach ($blogs as $index => $blog)
                            ...
                            <div class="x_blog-post mb-3 blog-post-item" data-title="{{ strtolower($blog->name) }}"
                                ...
                                @php
                                    $shortDesc = Str::words($blog->description, 20, '...');
                                @endphp
                                <p class="x_blog-desc" id="desc-{{ $index }}">
                                    {{ $shortDesc }}
                                </p>
                                <span class="show-more-link" onclick="toggleDescription({{ $index }})"
                            ...
                            @foreach ($topArticle as $article)
                                <div class="d-flex align-items-center mb-3">
                                    <img src="{{ asset('images/articles/' . $article->image) }}"
                                        class="x_blog-top-thumb me-3" alt="{{ $article->name }}" width="50">
                                    <div>
...
@push('script')
    <script>
        ...
        function toggleDescription(index) {
            ...
            const fullDesc = descEl.parentElement.getAttribute('data-desc');
            const shortDesc = fullDesc.split(' ').slice(0, 20).join(' ') + '...';

            const isExpanded = linkEl.innerText === 'Show Less';

            if (isExpanded) {
                descEl.innerText = shortDesc;
                linkEl.innerText = 'Show More';
            } else {
                descEl.innerText = fullDesc;
                linkEl.innerText = 'Show Less';
            }
        }
    </script>
    @endpush
